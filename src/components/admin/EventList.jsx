...
import { useGetEventManagersByEventIdQuery } from "../../api/eventApi";

export default function EventList({ events, onEdit, onDelete, onView, formatDate, formatTimeRemaining, getStatusText }) {
  if (events.length === 0) {
    return (
  ...
  function EventManagerName({ eventId }) {
    // Lấy danh sách manager theo eventId
    const { data: managerRoles = [] } = useGetEventManagersByEventIdQuery(eventId);
    ...
    if (!managerUserId) return <span style={{ color: '#888', fontSize: 12 }}>Chưa có quản lý</span>;
    return <span>{managerUserName}</span>;
  }
      ...
      {events.map(event => (
        <div className="event-card" key={event.id}>
          <div className="event-header">
            <div className="event-title">{event.title_short || event.title}</div>
          ...
          <div className="event-body">
            ...
            <div className="event-extra">
              {event.enable !== undefined && (
                <div><b>Trạng thái:</b> {event.enable ? "Hiển thị" : "Đã ẩn"}</div>
              )}
              {event.created_by !== undefined && event.created_by !== null && (
                <div><b>Người tạo:</b> <CreatorName userId={event.created_by} /></div>
              )}
              {/* Hiển thị tên người quản lý ngay dưới phần người tạo */}
              <div><b>Quản lý:</b> <EventManagerName eventId={event.id} /></div>
              {event.created_at && (
              ...
              {event.participants !== undefined && event.participants !== null && event.participants > 0 && (
                <div><b>Đã đăng ký:</b> {event.participants}</div>
              )}
            </div>
            <div className="event-actions" style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginBottom: 0, paddingBottom: 0 }}>
              <button className="btn btn-accent btn-small" onClick={() => onEdit(event)}>✏️ Sửa</button>
              <button className="btn btn-primary btn-small" onClick={() => onDelete(event.id)}>🗑️ Xóa</button>
              <button className="btn btn-outline btn-small" onClick={() => onView(event.id)}>👁️ Xem chi tiết</button>
            </div>
          </div>
        </div>
      ))}
