...
const OverviewTab = ({ eventData, stats = {} }) => {
  ...
  // Sử dụng fallback cho các giá trị thống kê
  const totalRegistered = stats?.totalRegistered ?? 0;
  const checkedIn = stats?.checkedIn ?? 0;
  const maxParticipants = eventData?.maxParticipants ?? 0;
  const percentRegistered = maxParticipants > 0 ? Math.round((totalRegistered / maxParticipants) * 100) : 0;
            ...
            <div className="space-y-3">
              ...
              <div className="mt-4 h-2 w-full rounded-full bg-gray-200">
                <div
                  ...
                  style={{
                    width: `${maxParticipants > 0 ? (totalRegistered / maxParticipants) * 100 : 0}%`,
                  }}
                ></div>
              </div>
              <p className="text-center text-sm text-gray-500">
                {percentRegistered}% đã đăng ký
              </p>
            </div>
          </div>
