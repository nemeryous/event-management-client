...
export default function DonViTenDonViManagement() {
  const { data: allDonVis = [], isLoading, error, refetch } = useGetAllDonViQuery();
  ...
  const items = useMemo(() => {
    const list = Array.isArray(allDonVis) ? allDonVis : (allDonVis?.content || []);
    return list
  ...
  async function handleCreate(e) {
    ...
    try {
      await createDonVi({ donVi: selectedDonVi, tenDonVi: newTenDonVi.trim() }).unwrap();
      setNewTenDonVi("");
  ...
  async function handleSaveEdit(row) {
    try {
      await updateDonVi({ id: row.id, donVi: selectedDonVi, tenDonVi: editingTen.trim() }).unwrap();
      setEditingId(null);
      setEditingTen("");
  ...
  return (
    <div className="p-4 md:p-8">
      ...
      {message && (
        <div className={`mb-4 rounded px-4 py-2 ${messageType === "success" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
          {message}
        </div>
      )}
      ...
      <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-end">
        <div className="flex-1">
          <label className="mb-1 block text-sm font-medium text-gray-600">Đơn vị</label>
          <select
        ...
        <form className="flex-1" onSubmit={handleCreate}>
          <label className="mb-1 block text-sm font-medium text-gray-600">Thêm tên đơn vị</label>
          ...
          <div className="hidden md:block">
            <DataTable
              ...
              columns={[
                ...
                { key: "donVi", label: "Đơn vị", render: (r) => getDonViLabel(r.donVi) },
                {
                  key: "tenDonVi",
                  label: "Tên đơn vị",
                  render: (row) => (
                ...
                },
                {
                  ...
                  render: (row) => (
                    editingId === row.id ? (
                      <ActionButtons
                        actions={[
                          { label: "Lưu", color: "text-[#223b73]", onClick: () => handleSaveEdit(row) },
                          { label: "Hủy", color: "text-gray-500", onClick: () => { setEditingId(null); setEditingTen(""); } },
                        ]}
                      />
                    ) : (
                      <ActionButtons
                        actions={[
                          { label: "Sửa", icon: <FiEdit2 />, color: "text-[#223b73]", onClick: () => { setEditingId(row.id); setEditingTen(row.tenDonVi); } },
                          { label: "Xóa", icon: <FiTrash2 />, color: "text-[#c52032]", onClick: () => handleDelete(row) },
                        ]}
                      />
                    )
                  ),
          ...
          <div className="space-y-3 md:hidden">
            {items.map((row, idx) => (
              <div key={row.id} className="rounded-xl bg-white p-4 shadow">
                ...
                <div className="text-sm"><span className="font-semibold">Đơn vị:</span> {getDonViLabel(row.donVi)}</div>
                ...
                <div className="mt-2">
                  {editingId === row.id ? (
                    <div className="flex gap-2">
                      <button className="rounded bg-[#223b73] px-3 py-1 text-white" onClick={() => handleSaveEdit(row)}>Lưu</button>
                      <button className="rounded bg-gray-200 px-3 py-1" onClick={() => { setEditingId(null); setEditingTen(""); }}>Hủy</button>
                    </div>
                  ) : (
                    <div className="flex gap-2">
                      <button className="rounded bg-[#223b73] px-3 py-1 text-white" onClick={() => { setEditingId(row.id); setEditingTen(row.tenDonVi); }}>Sửa</button>
                      <button className="rounded bg-[#c52032] px-3 py-1 text-white" onClick={() => handleDelete(row)}>Xóa</button>
                    </div>
                  )}
                </div>
              </div>
            ...
            {items.length === 0 && (
              <div className="py-8 text-center text-gray-400">Chưa có tên đơn vị</div>
            )}
          </div>
        </div>
      )}
